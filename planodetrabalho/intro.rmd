...
conhecidos como *data centers*. Os si temas de software presentes nos *data
...
Para aproveitar as simplicidade de acesso a infraestrutura e escalabilidade
...
no impacto que o aumento de complexidade e funcionalidade que estes ambientes
vem experimento tem no desempenho dos sistemas que eles dão suporte. No restante
desta seção iremos explicar melhor esses ambientes de execução, bem como
caracterizar melhor o problema a ser estudados e os objetivos desse trabalho.
...
Modelos de execução também pode existir independentemente de linguagens de 
...
Frequentemente não há critérios claros para decidir qual o comportamento de
idioma é considerado dentro do sistema de execução contra a qual o comportamento
é "compilada". Como um exemplo simples temos o ambiente de execução da linguagem
C [@Kernighan:1988:CPL:576122]. Esse ambiente de execução composto por um 
conjunto particular de instruções que são inseridas pelo compilador no programa
executável. Entre outras coisas, estas instruções gerenciam a pilha processador,
criam espaço para as variáveis locais e copiam parâmetros da chamadas de funções
para o topo da pilha. No outro extremo ambientes de execução podem prestar
serviços de uma ou máquina virtual, que se escondem até mesmo o conjunto de
instruções do processador. Esta é a abordagem seguida por muitas linguagens
interpretadas, tais como AWK [@Aho:1987:APL:29361] e algumas linguagens como
Java [@Arnold:2000:JPL:556709], que são compiladas em um código de representação
intermediária, o qual é independente de arquitetura do computador que vai ser executado (bytecode).
...
* Fluxo de entrada / saída: em sistemas operacionais como o Unix, que não têm
...
* Chamadas do sistema operacional chama: funcionalidades do sistema operacional
