...
t <- read.csv("../data/4cores/server/responses.200.csv")  %>% filter(t >= min(gc_cpu$t) & t <= max(gc_cpu$t))
req <- read.csv("../data/4cores/server/request.csv")  %>% filter(t >= min(gc_cpu$t) & t <= max(gc_cpu$t))
```
# 2 Cores
