...
knitr::opts_chunk$set(fig.asp = 0.3)
...
```{r}
latency <- read.csv(get_fname("latency"))
...
plotLatency <- function(df) {
  ...
  print(
    ggplot(df, size = 1) +
      ...
      geom_line(aes(x = (ts-startTime)/1000, y = p999, color = "99.9")) +
      ylab("Latency (ms)") +
      xlab("Time (s)") +
      scale_color_manual(values = COLOR_BLIND_PALETTE) +
      scale_y_continuous(breaks = seq(0, max(df$p999), by = 10)) +
      theme(
...
```{r}
tp <- read.csv(get_fname("tp"))
tp <- tp[(START_OFFSET+1):(nrow(tp)-END_OFFSET),]
tp["tsrange"] <- c(NA, (tp$ts[2:nrow(tp)] - tp$ts[1:(nrow(tp)-1)])/1000)
tp["value"] <- c(NA, (tp$count[2:nrow(tp)] - tp$count[1:(nrow(tp)-1)]) / tp$tsrange[2:nrow(tp)])
